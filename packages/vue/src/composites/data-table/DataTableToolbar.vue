...
const isPluginFilter = (type: string) =>
    !isBuiltInFilterType(type) && pluginMap.value.has(type);
 ...
 */
function supportsMultiInstance(
    filterType: string,
...
const addFilter = (fieldId: string) => {
    ...
    if (!supportsMultiInstance(field.type, props.advancedMode)) {
        ...
        const existingInstance = filterInstances.value.find(
            (i) => i.fieldId === fieldId,
        );
        if (existingInstance) {
...
const handleInstanceValueChange = (
    instanceId: string,
    value: FilterValue | undefined,
) => {
    const instance = filterInstances.value.find(
 ...
 */
const aggregateAndSetColumnFilter = (
    fieldId: string,
    filterType: FilterType,
) => {
    const instances = filterInstances.value.filter(
        (i) => i.fieldId === fieldId,
    );
...
const removeFilterInstance = (instanceId: string) => {
    const index = filterInstances.value.findIndex(
        (i) => i.instanceId === instanceId,
    );
    if (index === -1) return;
