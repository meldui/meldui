...
> **Full docs**: see `apps/docs/src/content/docs/document-viewer/` for the complete reference site (overview, getting started, every plugin, every use case, programmatic API, theming, bundle/perf, troubleshooting, and migration guides).
...
EmbedPDF ships PDFium as a WebAssembly binary inside `@embedpdf/pdfium`. Copy it into your public dir at build time and point DocumentViewer at it via `wasmUrl`:
...
See `openspec/changes/document-viewer/` for the canonical implementation status.
