...
/**
 * PdfController — internal slot child that lives inside `<EmbedPDF>` so
 ...
 *   3. Emit reactive state up to PdfViewer for the toolbar
 *
 * This component renders nothing — it's pure orchestration. Layout lives
 * in PdfViewer's `<Scroller>` slot.
 */
...
import CommentMarker from '../comments/CommentMarker.vue'
import type { AnnotationTransferItem as PdfAnnotationTransferItem } from '@embedpdf/plugin-annotation'
import { SpreadMode } from '@embedpdf/plugin-spread'
import { annotationToPdf, pdfToAnnotation } from '../utils/annotationMapping'
import { buildCommands, type CommandCallbacks } from '../composables/useCommands'
import type {
  AnnotationFilter,
...
interface Props {
  documentId: string
  /**
   * Callbacks for every shortcut-bound command DocumentViewer registers. The
   * controller is inside the EmbedPDF tree (so it has access to the
   * Commands plugin capability), but most of the actions need DocumentViewer-
  ...
  (scope) => {
    ...
    unsubscribeAnnotationEvent = scope.onAnnotationEvent((event) => {
      ...
      if (event.type === 'create') {
        const annotation = pdfToAnnotation(event.annotation)
        if (annotation) {
          ...
          if (annotation.type === 'highlight' && !annotation.selectedText && lastSelectedText) {
            annotation.selectedText = lastSelectedText
            lastSelectedText = ''
          }
          emit('annotation-created', { annotation: annotation })
        }
      } else if (event.type === 'update') {
        const annotation = pdfToAnnotation(event.annotation)
        if (annotation) {
          const patch = pdfToAnnotation({
            ...event.annotation,
            ...event.patch,
          } as PdfAnnotationObject) as Partial<Annotation>
          emit('annotation-updated', { annotation: annotation, patch })
        }
...
// every selected annotation. PdfViewer's slot relays select /
// deselect events into these methods.
let lastSelectedUid: string | null = null

/**
 * Map an EmbedPDF subtype to the Annotation discriminator so the host
 * can route on `type` even when a full annotation can't be resolved.
 */
function subtypeToAnnotationType(subtype: number | undefined): Annotation['type'] | null {
...
function emitSlotSelection(uid: string, subtype: number) {
  ...
  // fall back to a minimal `{ id, type }` stub so DocumentViewer can match its
  ...
  if (tracked) {
    const annotation = pdfToAnnotation(tracked.object)
    if (annotation) {
      emit('annotation-selected', { annotation: annotation })
      return
    }
  }
  const annotationType = subtypeToAnnotationType(subtype)
  emit('annotation-selected', {
    annotation: { id: uid, type: annotationType ?? 'highlight' } as unknown as Annotation,
  })
}
...
//     selected (correct).
//   - click inside the floating tooltip → guarded by `.highlight-tooltip`,
//     we leave the selection alone.
const HIGHLIGHT_TOOLTIP_SELECTOR = '.highlight-tooltip'
...
// ───────────────────────────────────────────────────────────────────────
// Imperative controller — invoked by DocumentViewer in response to toolbar events
// ───────────────────────────────────────────────────────────────────────
...
function createAnnotation(input: CreateAnnotationInput): Annotation {
  const id =
    input.id ??
    (typeof crypto !== 'undefined' && 'randomUUID' in crypto
      ? crypto.randomUUID()
      : `ann_${Date.now()}_${Math.random().toString(36).slice(2)}`)
  const created = { ...input, id } as Annotation
  const pdf = annotationToPdf(created)
...
function updateAnnotation(id: string, patch: Partial<Annotation>): void {
  ...
  if (!existing) return
  const merged = { ...pdfToAnnotation(existing.object), ...patch } as Annotation
  const pdfPatch = annotationToPdf(merged) as Partial<PdfAnnotationObject>
...
function getAnnotations(filter?: AnnotationFilter): Annotation[] {
  const scope = annotation.provides.value
  if (!scope) return []
  ...
  for (const t of tracked) {
    const annotation = pdfToAnnotation(t.object)
    if (!annotation) continue
    if (filter?.types && !filter.types.includes(annotation.type)) continue
    if (filter?.authorId && annotation.author !== filter.authorId) continue
    out.push(annotation)
  }
  return out
}

function importAnnotations(items: AnnotationTransferItem[]): void {
  const scope = annotation.provides.value
  if (!scope) return
  // Strip Vue reactive proxies before mapping — see the comment on
  // `loadAnnotations` for why this matters. `annotationToPdf` does shallow
  // copies, so without cloning here a proxied annotation would still
  // hand proxied inner objects to `scope.importAnnotations`.
  const mapped: PdfAnnotationTransferItem[] = items.map((item) => ({
    annotation: annotationToPdf(JSON.parse(JSON.stringify(item.annotation)) as Annotation),
    ctx: item.ctx,
  })) as PdfAnnotationTransferItem[]
  scope.importAnnotations(mapped)
}

async function exportAnnotations(filter?: AnnotationFilter): Promise<AnnotationTransferItem[]> {
  const scope = annotation.provides.value
  if (!scope) return []
  return new Promise((resolve, reject) => {
    scope
      ...
      .wait(
        (items) => {
          ...
          for (const it of items) {
            const annotation = pdfToAnnotation(it.annotation)
            if (!annotation) continue
            if (filter?.types && !filter.types.includes(annotation.type)) continue
            if (filter?.authorId && annotation.author !== filter.authorId) continue
            out.push({ annotation: annotation, ctx: it.ctx as never })
          }
          resolve(out)
        },
        (err) => reject(err),
 ...
 * plain `Annotation[]` shape consumers usually persist (rather than the
 * transfer-item wrapper).
 *
 * `DocumentViewer` calls this as soon as the renderer mounts, but the EmbedPDF
 ...
 */
const pendingLoad: { ref: Annotation[] | null } = { ref: null }

function loadAnnotations(annotations: Annotation[]): void {
  ...
  // story). `annotationMapping.annotationToPdf` is shallow, so without this
  ...
  const plain = JSON.parse(JSON.stringify(annotations)) as Annotation[]
...
watch(
  ...
  ([cap, callbacks, enabled]) => {
    ...
    for (const command of buildCommands(callbacks)) {
...
defineExpose({
  ...
  canUndo,
  canRedo,
  // selection signal (driven by SelectionSignal in PdfViewer)
  emitSlotSelection,
  emitSlotDeselection,
})
