...
/**
 * PdfViewer — wraps EmbedPDF with the plugins resolved from
 * DocumentViewer's feature config. Lazy-loaded by DocumentViewer for PDF documents
 * so non-PDF consumers never pay the PDFium WASM cost.
 *
 * Hosts a `<PdfController>` inside the `<EmbedPDF>` slot so plugin
 * composables can resolve their providers. The controller is re-exposed
 * via `defineExpose` so DocumentViewer can drive it from toolbar events.
 *
 * Side panels (outline, thumbnails) are mounted INSIDE the EmbedPDF tree
 * because they rely on plugin composables. DocumentViewer controls their
...
import PdfController from './PdfController.vue'
import HighlightTooltip from '../highlights/HighlightTooltip.vue'
...
interface Props {
  ...
  wasmUrl?: string
  /** Run PDFium in a Web Worker. Defaults to `true` — see `DocumentViewerProps.worker`. */
  worker?: boolean
  features?: ViewerFeatures
  featureConfig?: FeatureConfig
  // Panel visibility (driven by DocumentViewer)
  isOutlineOpen?: boolean
  isThumbnailsOpen?: boolean
  currentPage?: number
  // Shortcut-bound command callbacks (driven by DocumentViewer).
   ...
   * instead of forwarding the click to EmbedPDF. DocumentViewer then renders a
   * single `CommentForm` at the coord.
   */
  pendingCommentMode?: boolean
  /**
   * Current pending-comment-form coord, owned by DocumentViewer. When set, we
...
const emit = defineEmits<{
  ...
  /**
   * Inline reply submitted from a highlight's floating tooltip. DocumentViewer
   ...
   * DocumentViewer opens the annotations side panel and focuses the row for
   ...
   * along with the page wrapper's rendered width/height so DocumentViewer can
  ...
  /** Pending comment form submitted; DocumentViewer creates the annotation. */
...
function handleHighlightColorChange(_pageIndex: number, id: string, color: string) {
  controllerRef.value?.updateAnnotation(id, { color } as Partial<Annotation>)
}
 ...
 * imported annotations in 2.14.2). DocumentViewer pipes the resulting id
 ...
 * DocumentViewer can render a single `<CommentForm>` at the spot.
 *
...
// Capture controller ref so DocumentViewer can drive it via this renderer's
// re-exposed defineExpose.
const controllerRef = ref<InstanceType<typeof PdfController> | null>(null)

// Stash for `loadAnnotations` calls that arrive before the controller is
// mounted (the most common case — DocumentViewer pushes `initialAnnotations`
...
let pendingLoadAnnotations: Annotation[] | null = null
...
defineExpose({
  ...
  createAnnotation: (input: CreateAnnotationInput): Annotation => {
    ...
    if (!created) throw new Error('PdfViewer: controller not ready')
    return created
  },
  ...
  // DocumentViewer calls this as soon as the renderer mounts, but the controller
...
<template>
  <div class="pdf-viewer relative flex h-full flex-1 overflow-hidden">
    <div
      v-if="isLoading"
              ...
              <div v-if="isLoaded" class="flex h-full w-full">
                ...
                >
                  <!--
                    ...
                    side panel is hidden. `ViewerSidePanel` uses `v-show`
                  ...
                  >
                    <Scroller :document-id="coreState.activeDocumentId">
                      <template #default="{ page }">
                        ...
                        -->
                        <div
                          class="pdf-page relative bg-white shadow-sm select-none"
                        ...
                        >
                          <!--
                            ...
                            flow. Active only when DocumentViewer flips us into
                          ...
                          <div
                            ...
                            class="comment-overlay absolute inset-0 z-20 cursor-crosshair"
                            :data-page-index="page.pageIndex"
                            @pointerdown.capture="
                          ...
                          <!--
                            ...
                            (in `CommentForm` itself) can `stopPropagation`
                                ...
                                >
                                  ...
                                  <div v-if="selected" v-bind="menuWrapperProps">
                                    <!--
                                      ...
                                      DocumentViewer can track the active row in
