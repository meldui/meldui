...
/**
 * HighlightTooltip — floating affordance shown over a selected highlight.
 *
...
//
// DocumentViewer.vue provides a reactive Map<annotationId, CommentThread>. We pull
// the latest reply out for the hover tooltip on the comment icon. This is the
// same pattern `CommentMarker.vue` uses for sticky-note pins.
const threadsByAnnotationId = inject<Ref<ReadonlyMap<string, CommentThread>> | null>(
  THREADS_INJECT_KEY,
  null,
)
...
<template>
  <TooltipProvider :delay-duration="200">
    <div
      ref="rootRef"
      class="highlight-tooltip flex flex-col gap-1 rounded-md border border-border bg-popover p-1 text-popover-foreground shadow-md"
      role="toolbar"
    ...
    >
          ...
          <TooltipTrigger as-child>
            ...
            >
              <!--
                ...
                `CommentMarker.vue`'s reply-count badge for consistency.
              -->
      ...
      <!--
        ...
        `@pointerdown.stop` and the `.highlight-tooltip` class on the
        outer wrapper keep PdfController's outside-click-deselect handler
