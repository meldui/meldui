...
/**
 * DocumentViewer — top-level entry component.
 *
 ...
 * the EmbedPDF tree). DocumentViewer holds the open-state and forwards it as a prop.
 *
 ...
 * (PDF via EmbedPDF, plus text & markdown via `useTextSearch`). Keeps
...
// Updates flow from the renderer's onAnnotationEvent → DocumentViewer's
 ...
 * but hasn't yet committed a comment. While on, PdfViewer captures
 ...
 * single `<CommentForm>` at the spot — clicking elsewhere just updates
...
function handleSetTool(tool: string | null) {
  ...
  // path. Instead we flip DocumentViewer into `pendingCommentMode` and capture
...
async function handlePrint() {
  ...
  try {
    ...
    console.error('[DocumentViewer] print failed:', message)
  }
}
function handleFullscreen() {
  // Always fullscreen the DocumentViewer root so the toolbar stays visible. The
...
// ─────────────────────────────────────────────────────────────────────────
// Renderer → DocumentViewer state updates (PDF only)
// ─────────────────────────────────────────────────────────────────────────
...
function handleRendererAnnotationCreated(payload: { annotation: Annotation }) {
...
function handleRendererAnnotationSelected(payload: { annotation: Annotation | null }) {
 ...
 * (`HighlightTooltip` add-reply event). Posts to the same thread store
...
async function handleCommentFormSubmit(content: string) {
  ...
  if (isPdf.value && pdfRendererRef.value) {
    ...
    // 24×24 visual marker in CommentMarker.vue.
...
function handleNavigateToAnnotation(annotation: Annotation) {
  if (isPdf.value) {
...
function handlePanelUpdate(payload: { annotationId: string; patch: { contents: string } }) {
  if (isPdf.value) {
    pdfRendererRef.value?.updateAnnotation(
      ...
      payload.patch as Partial<Annotation>,
    )
  }
}
...
const threadList = computed<CommentThread[]>(() => Array.from(threads.threads.values()))

/**
 * Inject the live thread map into descendants so the custom annotation
 * renderer (CommentMarker — registered deep inside the EmbedPDF tree
 * via the renderer registry) can read reply counts and render the badge.
 */
const threadsByAnnotationId = computed<ReadonlyMap<string, CommentThread>>(
  () => new Map(threads.threads.entries()),
)
provide(THREADS_INJECT_KEY, threadsByAnnotationId)
...
/**
 * Command catalog passed to `PdfViewer → PdfController`. The
...
defineExpose<DocumentViewerInstance>({
  getDocumentInfo: () =>
  ...
  loadAnnotations: async (incoming: Annotation[]) => {
  ...
  createAnnotation: async (input: CreateAnnotationInput): Promise<Annotation> => {
    if (isPdf.value && pdfRendererRef.value) {
    ...
    const annotation = { ...input, id } as Annotation
    annotations.value = [...annotations.value, annotation]
    return annotation
  },

  updateAnnotation: async (id: string, patch: Partial<Annotation>) => {
    if (isPdf.value && pdfRendererRef.value) {
  ...
  loadThreads: (incoming: CommentThread[]) => threads.loadThreads(incoming),

  addReply: async (annotationId: string, content: string): Promise<CommentReply> => {
    const user = props.currentUser
 ...
 * created at runtime by `useTextSearch` and won't carry a Vue scope
 * attribute.
 */
