...
/**
 * AnnotationRow — single annotation row inside AnnotationsPanel.
 *
...
import CommentReplyForm from '../comments/CommentReplyForm.vue'
import { cn } from '../../../lib/utils'
import type { Annotation, CommentReply, CommentThread } from '../types'
...
function handleDeleteReply(reply: CommentReply) {
...
<template>
  <article
    ...
    :class="
      cn(
        'document-document-row flex flex-col gap-2 px-3 py-3 text-sm transition-colors hover:bg-muted/30',
    ...
    <div v-if="hasReplies" class="flex flex-col gap-2" @click.stop>
      ...
      <ul v-if="isRepliesExpanded" class="flex flex-col gap-2 border-l border-border pl-3">
        <li
          ...
          :key="reply.id"
          class="comment-reply flex flex-col gap-1"
        >
