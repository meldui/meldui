...
/**
 * AnnotationsPanel — side panel listing every annotation in the
 * document, grouped by page with sticky page headers.
 *
 * Differences from the previous `CommentsPanel`:
 *  - Title text is "Annotations" (set on the host `ViewerSidePanel`).
...
const items = computed(() => {
  const threadMap = new Map<string, CommentThread>(props.threads.map((t) => [t.annotationId, t]))
  const COMMENTABLE = new Set<Annotation['type']>(['highlight', 'comment', 'free-text'])
...
<template>
  <div class="annotations-panel flex h-full flex-col">
