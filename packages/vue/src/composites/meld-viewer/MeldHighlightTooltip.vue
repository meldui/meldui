...
const threadsByAnnotationId = inject<Ref<
    ReadonlyMap<string, MeldThread>
> | null>(MELD_THREADS_INJECT_KEY, null);
const thread = computed(
    () => threadsByAnnotationId?.value?.get(props.annotationId) ?? null,
);
...
const previewAuthor = computed<string>(
    () => latestReply.value?.authorName ?? "Anonymous",
);
...
function measureAndClamp() {
    ...
    const boundary = intersectBoundaries(
        pageRect,
    ...
    // Vertical: if the tooltip overflows below the boundary, flip it above
    // the highlight. Recover the highlight's height from the renderer
    // wrapper's inline `top: ${highlightHeight + 8}px`. A flip places the
    // tooltip at `bottom: highlightHeight + 8` above, which is a vertical
    // delta of -(highlightHeight + 2*gutter + tooltipHeight). If the
    // flipped position would itself overflow the boundary top, clamp to
    // the boundary top instead.
    let vShift = 0;
    if (naturalBottom > boundary.bottom - gutter) {
        const wrapper = el.parentElement as HTMLElement | null;
        const wrapperTopPx = wrapper
            ? Number.parseFloat(wrapper.style.top || "0") || 0
            : 0;
        ...
        if (flippedTop >= boundary.top + gutter) {
            vShift = flipDelta;
        } else {
            // Not enough room above either — clamp to the boundary top.
      ...
      -->
            ...
            >
                ...
                <div class="flex items-center justify-end gap-2">
                    <Button variant="ghost" size="sm" @click="cancelCommentForm"
                        >Cancel</Button
                    >
                    <Button
                        size="sm"
