/**
 * Command catalog for MeldViewer — registered with `@embedpdf/plugin-commands`
 ...
 * `useMeldKeyboard.ts` when EmbedPDF's Commands plugin is the canonical
 ...
 * `MeldPdfController` where the plugin capability is available.
 */
import type { Command } from '@embedpdf/plugin-commands'

/**
 * Callbacks for every command MeldViewer registers.
 *
 ...
 */
export interface MeldCommandCallbacks {
  // Page navigation
  prevPage: () => void
  nextPage: () => void
  firstPage: () => void
  ...
  // Search popover (MeldViewer-level UI state)
  openSearch: () => void
  // Panels (MeldViewer-level UI state)
  toggleComments: () => void
  toggleOutline: () => void
...
export function buildMeldCommands(cb: MeldCommandCallbacks): Command[] {
  return [
    {
      id: 'page.prev',
      label: 'Previous page',
