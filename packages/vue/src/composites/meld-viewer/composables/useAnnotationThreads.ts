 ...
 * `Map<annotationId, MeldThread>`. Consumers preload via `loadThreads()`,
...
import type { MeldReply, MeldThread, ThreadUpdatePayload } from '../types'

type ThreadStore = Map<string, MeldThread>
...
export function useAnnotationThreads() {
  ...
  const threads = reactive(new Map<string, MeldThread>()) as ThreadStore
  ...
  return {
    threads: readonly(threads) as ReadonlyMap<string, MeldThread>,
    subscribe,
    loadThreads,
    exportThreads,
    getThread,
