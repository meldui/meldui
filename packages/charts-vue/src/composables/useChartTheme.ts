...
export function useChartTheme() {
  ...
  const themeColors = computed(() => {
    ...
    return {
      ...
      info:
        style.getPropertyValue("--color-info")?.trim() || DEFAULT_PALETTE.info,
      purple:
        style.getPropertyValue("--color-purple")?.trim() ||
        DEFAULT_PALETTE.purple,
      pink:
        style.getPropertyValue("--color-pink")?.trim() || DEFAULT_PALETTE.pink,
      mutedForeground: resolveColor(
        style.getPropertyValue("--muted-foreground")?.trim() ||
          DEFAULT_TEXT_COLORS.mutedForeground,
      ),
      foreground: resolveColor(
        style.getPropertyValue("--foreground")?.trim() ||
          DEFAULT_TEXT_COLORS.foreground,
      ),
    };
  });
