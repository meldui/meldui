  ...
  >
      ...
      <div class="flex-1 flex items-end justify-center py-8 min-h-[200px]">
        <div class="flex items-end justify-evenly w-full h-full gap-4">
          <div
            ...
            :style="{ height: `${height}%` }"
          >
            <Skeleton :class="cn('w-full h-full rounded-t-md', !animated && 'animate-none')" />
          </div>
        </div>
      </div>
      <!-- Axis labels -->
    ...
    <template v-else-if="skeletonType === 'pie'">
      <div class="flex-1 flex items-center justify-center min-h-[200px]">
        <div class="relative w-[180px] h-[180px]">
          ...
          <svg class="absolute inset-0" width="180" height="180" viewBox="0 0 180 180">
            <line
              x1="90"
              y1="90"
              x2="90"
    ...
    <template v-else-if="skeletonType === 'scatter'">
      <div class="flex-1 relative min-h-[200px]">
        <!-- Grid lines -->
        <div class="absolute inset-4 border-l border-b border-muted-foreground/20" />
        <!-- Scatter dots -->
        <div class="absolute inset-4 h-full w-full">
          <Skeleton
            v-for="(dot, i) in scatterDots"
            :key="i"
            :class="cn('absolute rounded-full', !animated && 'animate-none')"
            :style="{
              left: `${dot.x}%`,
              top: `${dot.y}%`,
              width: `${dot.size}px`,
