...
import type { ChartEmits, MeldPieChartProps } from '../types'
import MeldChartSkeleton from './MeldChartSkeleton.vue'

const props = withDefaults(defineProps<MeldPieChartProps>(), {
  height: 350,
  width: '100%',
  loading: false,
})
...
const computedAriaLabel = computed(() => {
  ...
  return props.title
    ? `Pie chart: ${props.title}. Data series: ${seriesNames}`
    : `Pie chart with data series: ${seriesNames}`
})

// Transform config to ECharts options
const echartsOptions = computed(() =>
  transformToEChartsOption(props.config, chartThemeConfig.value, 'pie'),
)
