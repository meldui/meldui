...
import type { ChartEmits, MeldScatterChartProps } from '../types'
import MeldChartSkeleton from './MeldChartSkeleton.vue'

const props = withDefaults(defineProps<MeldScatterChartProps>(), {
  height: 350,
  width: '100%',
  loading: false,
})
...
const echartsOptions = computed(() =>
  transformToEChartsOption(props.config, chartThemeConfig.value, 'scatter'),
)
...
watch(
  ...
  (newConfig) => {
    if (chartInstance.value) {
      const newOptions = transformToEChartsOption(newConfig, chartThemeConfig.value, 'scatter')
...
watch(
  ...
  (newTheme) => {
    if (chartInstance.value) {
      const newOptions = transformToEChartsOption(props.config, newTheme, 'scatter')
      updateChart(newOptions)
    }
  },
  { deep: true },
