...
import type { ChartEmits, MeldDonutChartProps } from '../types'
import MeldChartSkeleton from './MeldChartSkeleton.vue'

const props = withDefaults(defineProps<MeldDonutChartProps>(), {
  height: 350,
  width: '100%',
  loading: false,
})
...
const computedAriaLabel = computed(() => {
  ...
  return props.title
    ? `Donut chart: ${props.title}. Data series: ${seriesNames}`
    : `Donut chart with data series: ${seriesNames}`
})

// Transform config to ECharts options
const echartsOptions = computed(() =>
  transformToEChartsOption(props.config, chartThemeConfig.value, 'donut'),
)
...
<template>
  <div class="flex flex-col" :style="{ width: computedWidth }">
    ...
    >
      ...
      <MeldChartSkeleton v-if="!chartReady || loading" :height="height" :width="width" type="pie" />
