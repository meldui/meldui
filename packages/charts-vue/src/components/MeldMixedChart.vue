...
import type { ChartEmits, MeldMixedChartProps } from '../types'
import MeldChartSkeleton from './MeldChartSkeleton.vue'

const props = withDefaults(defineProps<MeldMixedChartProps>(), {
  height: 350,
  width: '100%',
  loading: false,
})
...
const echartsOptions = computed(() =>
  transformToEChartsOption(props.config, chartThemeConfig.value, 'mixed'),
)
...
watch(
  ...
  (newConfig) => {
    if (chartInstance.value) {
      const newOptions = transformToEChartsOption(newConfig, chartThemeConfig.value, 'mixed')
...
watch(
  ...
  (newTheme) => {
    if (chartInstance.value) {
      const newOptions = transformToEChartsOption(props.config, newTheme, 'mixed')
      updateChart(newOptions)
    }
  },
  { deep: true },
