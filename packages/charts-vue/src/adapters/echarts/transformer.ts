 ...
 */
function buildRadarConfig(config: NormalizedConfig, colors: ResolvedTextColors): EChartsConfig {
 ...
 */
function buildHeatmapConfig(resolvedColors: string[], colors: ResolvedTextColors): EChartsConfig {
  return {
    visualMap: {
      min: 0,
      max: 100,
...
export function transformToEChartsOption(
  ...
  const builtXAxis = buildXAxis(xAxis, chartType, horizontal, textColors.mutedForeground)
  const builtYAxis = buildYAxis(yAxis, chartType, horizontal, textColors.mutedForeground)
  ...
  const echartsOption = {
    ...
    legend: {
      ...
      ...(textColors.foreground && {
        textStyle: {
          ...((_defaultLegend as Record<string, unknown>)?.textStyle as Record<string, unknown>),
    ...
    ...(chartType === 'radar' && buildRadarConfig({ xAxis, yAxis, grid, series }, textColors)),

    // Heatmap specific configuration - visualMap is required for heatmaps
    ...(chartType === 'heatmap' && buildHeatmapConfig(resolvedColors, textColors)),
  }
