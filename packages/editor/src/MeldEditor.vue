...
import type { MeldEditorProps, MeldEditorEmits, MeldEditorExposed } from './types'
import {
...
const resolvedToolbarItems = computed(() => props.toolbarItems ?? createDefaultToolbarItems())

// Resolve extensions
const resolvedExtensions = computed(() => {
  if (props.overrideExtensions) return [...props.overrideExtensions, ...customExtensions.value]
...
<template>
  ...
  >
    ...
    <slot name="toolbar" :editor="editor" :items="resolvedToolbarItems">
      <EditorToolbar v-if="showToolbar && editor" :editor="editor" :items="resolvedToolbarItems" />
    </slot>
    ...
    >
      ...
      <slot name="bubble-menu" :editor="editor">
        <TextBubbleMenu v-if="showBubbleMenu && editor" :editor="editor" />
      </slot>

      <!-- Table controls -->
      <TableControls v-if="editor && editable" :editor="editor" :container-el="editorBodyRef" />
    </div>
...
/* @tiptap/vue-3 3.23+ wraps NodeViewContent children in an extra element.
   Make it transparent so the .column items remain direct grid items
   (otherwise the columns stack vertically). */
:deep(.column-content > [data-node-view-content-vue]) {
  display: contents;
}
