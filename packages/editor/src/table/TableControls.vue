...
function openMenu(type: 'row' | 'col', index: number, e: MouseEvent) {
  if (type === 'row') focusCell(index, 0)
  else focusCell(0, index)
...
<template>
  <div
    v-show="visible"
    class="table-controls-root absolute top-0 left-0 pointer-events-none z-[15]"
  >
    <!-- Column handles -->
    <button
      ...
      :class="{
        'opacity-70 pointer-events-auto':
          hoveredCol === i || (menu?.type === 'col' && menu.index === i),
      }"
    ...
    <button
      ...
      :class="{
        'opacity-70 pointer-events-auto':
          hoveredRow === i || (menu?.type === 'row' && menu.index === i),
      }"
  ...
  <Teleport to="body">
    ...
    >
      <template v-if="menu.type === 'row'">
        <button
          class="flex w-full items-center gap-2.5 rounded-md px-3 py-1.5 text-sm text-popover-foreground hover:bg-accent"
        ...
        <button
          class="flex w-full items-center gap-2.5 rounded-md px-3 py-1.5 text-sm text-destructive hover:bg-destructive/10"
