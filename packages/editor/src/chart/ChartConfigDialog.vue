...
watch(
  ...
  (val) => {
    ...
    const rawCategories = ((props.config.xAxis as Record<string, unknown>)?.categories ??
      []) as string[]
...
function addSeries() {
  series.value.push({
    ...
    data: Array(categories.value.length).fill(0),
  })
}
...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="sm:max-w-lg max-h-[85vh] overflow-y-auto">
      ...
      <!-- Title (optional) -->
      <Input v-model="chartTitle" placeholder="Chart title (optional)" class="text-sm" />
          ...
          <div v-for="(s, si) in series" :key="si" class="space-y-2">
            ...
            <div class="grid grid-cols-3 gap-1.5">
              <div v-for="(_, di) in s.data" :key="di" class="flex items-center gap-1">
                <span class="text-xs text-muted-foreground w-8 shrink-0 text-right truncate">{{
                  categories[di] ?? di + 1
                }}</span>
                <Input
