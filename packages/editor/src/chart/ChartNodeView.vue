  ...
  <NodeViewWrapper class="py-4">
    <div ref="containerRef" class="relative">
      ...
      <template v-if="isConfigured">
        ...
        <MeldChart :type="chartType" :config="config" :title="chartTitle || undefined" />
      </template>

      <!-- Unconfigured: show placeholder -->
      <div
        v-else
        class="flex items-center justify-center rounded-lg border border-dashed border-border bg-muted/30 p-8 text-muted-foreground"
      >
    ...
    <ChartPickerDialog :open="pickerOpen" @update:open="onPickerClose" @select="onCategorySelect" />
