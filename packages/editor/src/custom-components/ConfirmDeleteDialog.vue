      ...
      <DialogHeader>
        <DialogTitle>
          ...
          <template v-else-if="isSingleType"
            >Delete {{ items[0]?.count }} {{ items[0]?.name }} blocks?</template
          >
          <template v-else>Delete {{ totalCount }} blocks?</template>
        </DialogTitle>
        <DialogDescription as="div">
          <template v-if="isSingleBlock">
            This will remove the {{ items[0]?.name }} block and all its data. This action cannot be
            undone.
          </template>
          <template v-else-if="isSingleType">
            This will remove {{ items[0]?.count }} {{ items[0]?.name }} blocks and all their data.
