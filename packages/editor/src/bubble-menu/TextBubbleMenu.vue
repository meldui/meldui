...
<template>
  ...
  >
    <TooltipProvider :delay-duration="400">
      <div
        class="flex items-center gap-0.5 rounded-lg border border-border bg-popover p-1 shadow-lg"
      >
          ...
          <Tooltip>
            ...
            <TooltipContent side="top" :side-offset="8">
              <p class="flex items-center gap-1.5">
                {{ item.label }}
                <kbd v-if="item.shortcut" class="text-xs text-muted-foreground opacity-70">{{
                  item.shortcut
                }}</kbd>
              </p>
            </TooltipContent>
