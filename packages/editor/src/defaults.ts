...
import { IconBold, IconItalic, IconH1, IconH2, IconList, IconListCheck } from '@meldui/tabler-vue'
...
export function createDefaultExtensions(options: {
  ...
  if (d.table !== false) {
    exts.push(
      typeof d.table === 'object' ? Table.configure(d.table) : Table.configure({ resizable: true }),
    )
