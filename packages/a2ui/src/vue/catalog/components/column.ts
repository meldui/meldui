...
const MeldColumn = defineComponent({
  name: 'MeldColumn',
  props: {
  ...
  setup(props) {
    return () => {
      ...
      // web_core resolves a static `children: ['id', ...]` to raw string ids and
      // a `{path, componentId}` template to `{id, basePath}[]`. Normalize both.
      ...
      return h(
        'div',
        { class: ['flex flex-col gap-2', justify, align], 'data-a2ui': 'Column' },
        children.map((c) =>
          h(DeferredChild, {
            key: `${c.id}@${c.basePath}`,
            surface,
...
export const columnEntry = defineVueComponent(ColumnApi, MeldColumn)
