...
### Requirement: Standalone Pagination Composite

A composite component `<Pagination>` SHALL be exported from `@meldui/vue` at the package root and from `composites/pagination/`. It MUST be usable independently of `<DataTable>` and MUST NOT depend on a TanStack `Table` instance or any other table-specific construct.

#### Scenario: Rendered as a sibling to a card grid

- **WHEN** a parent renders `<Pagination v-model:pagination="pagination" :page-count="5" :total-rows="48" />` alongside a custom card grid
...
- **THEN** `<DataTable>` internally renders the same `<Pagination>` component with the same prop shapes
- **AND** binding the same `pagination` ref to both DataTable and a sibling `<Pagination>` keeps them in sync via Vue reactivity

### Requirement: Pagination V-Model Shape

`<Pagination>` SHALL accept a `pagination: PaginationState` prop as its v-model target, where `PaginationState = { pageIndex: number; pageSize: number }`. It SHALL emit `update:pagination` with a new `PaginationState` value whenever the user interacts with a page button or the page-size selector. The component MUST NOT mutate the bound ref directly; mutations flow exclusively through the emit.
...
### Requirement: Pagination Display Props

`<Pagination>` SHALL accept the following display-only props which it does not mutate: `pageCount: number` (required), `totalRows?: number`, `pageSizeOptions?: number[]` (default `[10, 20, 30, 40, 50]`), `showPageSizeSelector?: boolean` (default `true`), `showPageInfo?: boolean` (default `true`), `showSelectedCount?: boolean` (default `false`).

#### Scenario: showPageSizeSelector false hides the dropdown

- **WHEN** `<Pagination :show-page-size-selector="false">` is rendered
- **THEN** the page-size `<Select>` is not present in the DOM

#### Scenario: pageSizeOptions populates the dropdown

- **WHEN** `<Pagination :page-size-options="[5, 25, 100]">` is rendered
- **THEN** the page-size dropdown lists exactly `5`, `25`, `100` as options

### Requirement: Pagination Button Disabled State

`<Pagination>` SHALL derive Prev/First button disabled state from `pagination.pageIndex === 0` and Next/Last button disabled state from `pagination.pageIndex >= pageCount - 1`. Disabled buttons MUST NOT emit `update:pagination` on click.
