...
- **THEN** MeldViewer renders the PDF using EmbedPDF with continuous virtualized scrolling
...
- **THEN** MeldViewer renders that document with the matching non-PDF renderer
...
- **THEN** MeldViewer renders the image with the image renderer
...
- **WHEN** the consumer mounts MeldViewer for a PDF document
...
- **WHEN** the consumer mounts MeldViewer for a PDF document without overriding the `worker` prop
...
- **WHEN** MeldViewer renders its toolbar
...
- **THEN** MeldViewer's chrome adapts using `dark:` Tailwind utilities, matching the existing viewer's dark-mode behaviour
...
- **WHEN** a developer audits the `meld-viewer` source for imports
- **THEN** no Inertia, Phoenix, Vue Router, or doqo-specific module appears

### Requirement: Component Documentation

The package SHALL ship an extensive documentation site under `apps/docs/src/content/docs/meld-viewer/` covering overview, getting started, every supported feature/plugin, every documented use case, customization, theming, bundle and performance, troubleshooting, and migration. Documentation is part of the Phase 1 release definition; an undocumented release is not acceptable.

#### Scenario: Top-level guides present

- **WHEN** the Phase 1 release is published
- **THEN** the following pages exist and render under `apps/docs/src/content/docs/meld-viewer/`: `index.mdx`, `getting-started.mdx`, `features.mdx`, `programmatic-api.mdx`, `annotations.mdx`, `customization.mdx`, `theming.mdx`, `bundle-and-perf.mdx`, `troubleshooting.mdx`, `migration.mdx`

#### Scenario: Per-plugin reference pages

- **WHEN** the Phase 1 release is published
- **THEN** the `meld-viewer/plugins/` subdirectory contains one MDX page for every EmbedPDF plugin in scope (Phase 1: document-manager, viewport, scroll, render, tiling, zoom, rotate, spread, pan, fullscreen, interaction-manager, selection, search, bookmark, thumbnail, export, hotkeys, annotation, history)
...
- **WHEN** the Phase 1 release is published
- **THEN** the `meld-viewer/use-cases/` subdirectory contains at minimum: `index.mdx`, `load-saved-annotations.mdx`, `ai-rag-citations.mdx`, `import-export-annotations.mdx`, `save-as-copy-burnin.mdx`, `share-link-viewer.mdx`, `multi-format-rendering.mdx`, `threaded-comments.mdx`, `keyboard-and-touch.mdx`, `large-documents.mdx`, `dark-mode-integration.mdx`, `toolbar-customization.mdx`
- **AND** each use case page embeds a runnable Vue demo via `client:only="vue"` from `apps/docs/src/demos/meld-viewer/`
- **WHEN** Phase 2 ships
...
- **THEN** TSDoc descriptions appear for `MeldViewerProps`, `MeldViewerFeatures`, `MeldViewerFeatureConfig`, `MeldViewerInstance`, `MeldAnnotation`, `MeldReply`, `Thread`, and every other exported type

#### Scenario: Pagefind indexing

- **WHEN** a docs reader uses the on-site search
- **THEN** every MeldViewer page is indexed and surfaces via Pagefind

#### Scenario: LLM-friendly export

- **WHEN** the `generate-llms.ts` script runs as part of `apps/docs/` build
- **THEN** every MeldViewer page is included in the output

#### Scenario: Storybook docs page

- **WHEN** the consumer browses the Storybook for `@meldui/vue`
- **THEN** an MDX docs page for MeldViewer renders with embedded live stories
- **AND** the page links out to the full `apps/docs/` MeldViewer site for deeper reference
