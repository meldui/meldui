...
The Search plugin is **always registered** — `useSearch` is always available. `features.search` controls whether the search popover (and the per-page `SearchLayer` that paints match rects) is shown.
