...
| Primitive      | Used by                                                          |
| -------------- | --------------------------------------------------------------- |
| `Button`       | Every toolbar button, search case / whole-word toggles, comment form actions, reply submit |
| `Popover`      | Search popover                                                  |
| `DropdownMenu` | Zoom presets, mobile overflow menu                             |
| `Tooltip`      | Highlight tooltip wrapper, button hover labels                 |
| `Textarea`     | Reply form (the comment-create form uses a raw `<textarea>`)   |
...
The 5 default highlight colours live in the exported `HIGHLIGHT_COLORS` constant, and the floating tooltip's swatches always render from it:
...
`featureConfig.annotations.defaultTools` exists on the type as the intended palette-override hook, but it is **not yet applied** by the viewer — the swatch palette is currently fixed to `HIGHLIGHT_COLORS`.
