...
If you set a **bare relative** `wasm-url` (e.g. `wasm-url="pdfium.wasm"` or `"./pdfium.wasm"`) and the engine runs in a Web Worker, the worker is built from a blob URL whose base differs from the page's, so the relative path resolves against the blob origin and 404s.

The viewer resolves `wasmUrl` against `document.baseURI` before handing it to the engine, so a **root-relative** path (`/pdfium.wasm`) and a fully-qualified URL both work with **either** mode. Only bare relative paths are affected.

Two fixes:

1. **Use a root-relative or fully-qualified URL**: `wasm-url="/pdfium.wasm"` or `wasm-url="https://your-app.com/pdfium.wasm"`
...
1. **Worker mode on?** With `:worker="false"` (the prop defaults to `true`), large documents block the main thread during initial decode.
...
1. The source is a **PDF**. Programmatic annotation loading only renders on the PDF renderer; for image / text / markdown the annotations are stored but not painted.
