...
It is built as a single composite with a `features` prop that toggles each capability's toolbar / UX. All Phase 1 plugins are always registered (the flags gate visibility, not registration); the Phase 2 editing plugins — stamps, signature, redaction, forms, attachments — are registered only when their flag is on, so those packages tree-shake out when unused. See [Bundle & Perf](/docs/document-viewer/bundle-and-perf) for the details.
