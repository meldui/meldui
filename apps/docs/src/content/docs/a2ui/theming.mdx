...
> The A2UI protocol has an optional `theme.primaryColor` an agent *could* send. MeldUI intentionally does **not** map it onto CSS — the host owns color so surfaces stay consistent across agents. A per-surface agent override is a deferred, opt-in future addition.
