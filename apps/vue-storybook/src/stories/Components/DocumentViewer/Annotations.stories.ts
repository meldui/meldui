/**
 * DocumentViewer — annotation system stories.
 *
...
const meta: Meta<typeof DocumentViewer> = {
  title: 'Components/DocumentViewer/Annotations',
...
Annotations are persisted via the programmatic API (\`DocumentViewerInstance\`)
  ...
  render: () => ({
    ...
    setup() {
      ...
      async function exportAll() {
        ...
        const threadList = annotationList.map((a) => v.getThread(a.id)).filter((t) => t !== null)
 ...
 *    by the viewer's top-level events (`document-*` + `thread-update`).
 ...
 *  - Mutate via the `DocumentViewerInstance` programmatic API (addReply,
...
export const ExternalPanel: Story = {
  render: () => ({
    ...
    setup() {
      ...
      const threads = ref<CommentThread[]>([...SEEDED_THREADS])
      ...
      function onCreated(payload: { annotation: Annotation }) {
      ...
      function onUpdated(payload: { annotation: Annotation }) {
      ...
      function onThreadUpdate(payload: { thread: CommentThread; action: string }) {
      ...
      function getThread(annotationId: string): CommentThread | undefined {
      ...
      const panelItems = computed(() =>
        annotations.value
          .filter((a) => a.type === 'highlight' || a.type === 'comment' || a.type === 'free-text')
          .map((a) => ({
            annotation: a,
            thread:
              getThread(a.id) ??
              ({ annotationId: a.id, isResolved: false, replies: [] } as CommentThread),
          })),
      )
      ...
      function importFromJson() {
        try {
          ...
          annotations.value = isWrapped
            ? (parsed as Array<{ annotation: Annotation }>).map((i) => i.annotation)
