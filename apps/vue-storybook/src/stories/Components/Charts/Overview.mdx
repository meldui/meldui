...
All chart types share a consistent configuration API:
...
interface MeldChartConfig {
  ...
  legend?: ChartLegend         // Legend settings
  tooltip?: ChartTooltip       // Tooltip customization
  grid?: ChartGrid             // Grid lines
  stroke?: ChartStroke         // Line styling
  colors?: string[] | string   // Color palette
  stacked?: boolean            // Stack series
  horizontal?: boolean         // Horizontal orientation
  advanced?: Record<string, any> // Escape hatch
}
```
...
|------|------|---------|-------------|
| `config` | `MeldChartConfig` | required | Chart configuration |
