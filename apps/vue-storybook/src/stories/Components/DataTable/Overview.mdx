...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>defaultFilters</code></td>
...
Use `defaultFilters` to restore filter state from URL parameters on page load:
...
const defaultFilters = computed<ColumnFiltersState>(() => {
...
This ensures the filter UI reflects the URL state when the page loads, avoiding double API calls.
