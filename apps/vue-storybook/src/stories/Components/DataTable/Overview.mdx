...
const initialFilters = computed<ColumnFiltersState>(() => {
  const filters: ColumnFiltersState = []
  if (route.query.status) {
    filters.push({ id: 'status', value: [route.query.status] })
  }
  if (route.query.role) {
    filters.push({ id: 'role', value: route.query.role })
  }
  return filters
})
</script>
