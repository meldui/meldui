...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Render filter UI inside the toolbar. When <code>true</code>, the DataTable instantiates{' '}
        <code>useFilters</code> internally and exposes it via{' '}
        <code>tableRef.value.filtersState</code>. When <code>false</code> (default), the parent owns
        filter state via a separate <code>&lt;Filters&gt;</code>
